...
export default class BitcoinLedgerDevice extends EventEmitter {
  ...
  async buildTx (path) {
    ...
    const result = await app.createPaymentTransactionNew([[tx1, 0]], [path], undefined, script)
  }
  ...
  _getDerivedWallet (derivedPath) {
    if (this.seed) {
      const wallet = bitcoin.HDNode.fromSeedBuffer(Buffer.from(this.seed.substring(2), 'hex'), bitcoin.networks.testnet).derivePath(derivedPath)
      return wallet
    }
  }
}
