...
import { createAccountEntry, createDeviceAccountEntry } from '@chronobank/core/redux/persistAccount/utils'
import * as LoginUINavActions from './navigation'
import { FORM_LOGIN_PAGE, FORM_LOGIN_PAGE_FIELD_SUCCESS_MESSAGE, FORM_FOOTER_EMAIL_SUBSCRIPTION } from './constants'
  ...
  switch (wlt.type) {
    case 'memory': {
      try {
        ...
        const { selectedAccount, selectedProviderId, selectedNetworkId } = getState().get(DUCK_NETWORK)

        dispatch(NetworkActions.clearErrors())
        dispatch(SessionThunks.createNetworkSession(selectedAccount, selectedProviderId, selectedNetworkId))
...
export const onSubmitImportAccount = ({ name, password, mnemonic = '', privateKey = '' }) => async (dispatch) => {
  try {
    const wallet = await dispatch(
      PersistAccountActions.createMemoryAccount({
        name,
        password,
        mnemonic,
        privateKey,
...
export const onSubmitResetAccountPasswordSuccess = () => (dispatch) => {
  ...
  dispatch(change(FORM_LOGIN_PAGE, FORM_LOGIN_PAGE_FIELD_SUCCESS_MESSAGE, 'Your password has been reset.'))
}
 ...
 */
export const onSubmitSubscribeNewsletterFail = (errors, submitErrors) => (dispatch) => {
